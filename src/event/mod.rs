...
fn rewrap_bad_reason(bad_status: BadStatusError) -> Error {
    let BadStatusError::BadValue(v) = bad_status;
    Error::BadReason(v)
}

fn rewrap_bd_addr_type_err(bad_addr_type: crate::BdAddrTypeError) -> Error {
...
impl Event {
    ...
    pub fn new(packet: Packet) -> Result<Event, Error> {
        ...
        match event_type {
            ...
            0xFF => Ok(Event::Vendor(VendorEvent::new(payload)?)),
            _ => Err(Error::UnknownEvent(event_type)),
        }
    }
}

fn to_le_meta_event(payload: &[u8]) -> Result<Event, Error> {
...
fn to_number_of_completed_packets(payload: &[u8]) -> Result<NumberOfCompletedPackets, Error> {
    require_len_at_least!(payload, 1);
...
impl LeAdvertisingReport {
    ...
    fn inner_iter(&self) -> LeAdvertisingReportInnerIterator<'_> {
...
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
struct LeAdvertisingReportInnerIterator<'a> {
    event_data: &'a [u8],
    next_index: usize,
}
...
impl<'a> LeAdvertisingReportInnerIterator<'a> {
    fn next(&mut self) -> Result<Option<LeAdvertisement<'a>>, Error> {
...
fn to_le_advertising_report(payload: &[u8]) -> Result<LeAdvertisingReport, Error> {
...
fn to_le_ltk_request(payload: &[u8]) -> Result<LeLongTermKeyRequest, Error> {
...
fn to_le_data_length_change_event(payload: &[u8]) -> Result<LeDataLengthChangeEvent, Error> {
    require_len!(payload, 11);
